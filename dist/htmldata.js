...
function htmlData(empArr) {
    let cards = "";
    for (let i = 0; i < empArr.length; i++){
      console.log(empArr[i])
        const role = empArr[i].getRole()
        let icon = '';
        let info = '';

        if (role === 'manager') {
            icon += `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cup-hot" viewBox="0 0 16 16">
          ...
          info += `<li class="list-group-item">Office: ${empArr[i].office}</li>`
          ...
          info += `<li class="list-group-item">School: ${empArr[i].school}</li>`
        }
      const card = ` <div class="card" style="width: 18rem;">
      <!-- <img src="..." class="card-img-top" alt="..."> -->
       <div class="card-body">
         <h5 class="card-title">${empArr[i].name}</h5>
         <p class="card-text">${empArr[i].getRole()}</p>
         ${icon}
       </div>
    ...
    <body>
        <header class="header row container-fluid">
            <h1>Employees</h1>
        </header>
